...
- **D1 — Save do Gmail é MERGE, não replace, e só com os existentes carregados.** `useSaveUserSources`/`replace_user_sources` substitui a seleção inteira. Para não apagar programas existentes, a CTA de Revisar salva `união(useUserSources.data, incluídos)` (dedup por `source_item.id`). **Pré-condição obrigatória:** o merge só roda com `useUserSources` **carregado com sucesso** (`!isLoading && !error`); enquanto carrega, a CTA fica em "Salvando…"/desabilitada; se `useUserSources` falhar, mostra erro com "Tentar novamente" (`refetch`) e **não** salva. Isso impede o merge rodar com `data` indefinido (o que zeraria os existentes via replace).
...
- **D3 — Conjunto demo determinístico (um algoritmo só).** Achados = as **3 primeiras fontes que tenham ≥1 `source_item`** na 1ª categoria retornada por `useSources` (ordem `sort_order` das fontes), pegando **o primeiro `source_item`** de cada (ordem `sort_order`). Fontes sem item são puladas. Se houver <3 fontes elegíveis, usa as que houver. Cada achado = 1 programa/variante; todos começam incluídos.
...
- **D9 — Vasculhando é componente novo, com tempos exatos.** `TransitionScreen` atual permanece só como transição de save do Wizard. Scan cosmético de **2400ms** (timer fixo), com 3 labels rotativos de 800ms cada: "Procurando seus programas…", "Cruzando com o catálogo…", "Montando seu radar…". Com `prefers-reduced-motion`, pula direto ao estado concluído (sem timer).
...
- `OnboardingPage` orquestra estados: `'welcome' | 'method' | 'manual' | 'gmail-scan' | 'gmail-review' | 'gmail-done'`. O `gmail-done` renderiza o **Radar montado** (mesma tela do caminho manual) com os achados incluídos, antes de ir para `/alertas` — os dois caminhos convergem em Radar montado → Alertas.
...
- Botão Voltar → Método (estado `'method'`); as escolhas de Revisar são efêmeras.
- CTA "Adicionar ao radar": **desabilitada com 0 incluídos** (D5) **e enquanto `useUserSources` ainda carrega** (D1); salva `união(existentes, incluídos)` via `useSaveUserSources`.
- **Estado de save:** durante o `mutateAsync`, CTA em "Salvando…" e desabilitada (sem duplo clique); em erro (do save **ou** de `useUserSources`), mensagem `role="alert"` "Não foi possível salvar. Tente de novo." (com `refetch` no caso de `useUserSources`) e as escolhas permanecem; em sucesso → Radar montado.
...
- Modo edição (via Perfil): "Voltar" → `/perfil`; grava cada mudança de toggle na hora e **recalcula** `optIn = (novos || prazo || resumo)` — ligar qualquer alerta reengaja (`optIn:true`); desligar todos → `optIn:false`. (No onboarding o `optIn` é explícito pelos botões; na edição é derivado dos toggles.)
...
- **E2E (Playwright):** estende `tests/e2e/app-layout.spec.ts` com o caminho Gmail Prévia (método → vasculhando → revisar → adiciona → radar → alertas → painel) nos 4 cenários, com no-overflow e screenshots. **Invariante de seed:** o `supabase/seed.sql` local precisa ter ≥1 fonte com ≥1 `source_item` na 1ª categoria (a mesma que alimenta D3); confirmar/ajustar o seed no plano se necessário.
...
- Sem TBD/placeholders; quantidades exatas (D3: 3 fontes elegíveis, 1º item de cada; scan 2400ms / 3 labels de 800ms).
- Consistência: "Adicionar ao radar" = merge não destrutivo **e guardado por `useUserSources` carregado** (D1); contagem cosmética (D2) elimina a contradição com `useSources`; `onView` por-caminho (D6) elimina a contradição manual/edit; D3 tem um único algoritmo.
- Escopo: uma etapa (App), um plano; contagem real e loop de bootstrap explicitamente fora.
- Ambiguidades resolvidas em D1–D9; estados de loading/erro/pending/empty definidos e testáveis; `optIn` explícito no onboarding e derivado na edição.
